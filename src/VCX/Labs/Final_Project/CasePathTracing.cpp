...
namespace VCX::Labs::Rendering {
    ...
    void CasePathTracing::OnSetupPropsUI() {
        ...
        if (ImGui::CollapsingHeader("Appearance", ImGuiTreeNodeFlags_DefaultOpen)) {
            ...
            //_resetDirty |= ImGui::Checkbox("KD-Tree Accelerate", &_enablekd);
        }
        ImGui::Spacing();
        ...
        if (! _stopFlag && ! _task.joinable()) {
            ...
            _task = std::thread([&]() {
                ...
                for (; _pixelIndex < totalPixels;) {
                    ...
                    float const  fovFactor = std::tan(glm::radians(camera.Fovy) / 2.0f);

                    glm::vec3 pixelSum(0.0f);

                    for (int sy = 0; sy < _superSampleRate; ++sy) {
                        for (int sx = 0; sx < _superSampleRate; ++sx) {
                            glm::vec3 subPixelSum(0.0f);
                            int       samplesPerSubPixel = 64;

                            for (int s = 0; s < samplesPerSubPixel; ++s) {
                                float r1 = 2.0f * glm::linearRand(0.0f, 1.0f);
                                float tx = (r1 < 1.0f) ? (std::sqrt(r1) - 1.0f) : (1.0f - std::sqrt(2.0f - r1));

                                float r2 = 2.0f * glm::linearRand(0.0f, 1.0f);
                                float ty = (r2 < 1.0f) ? (std::sqrt(r2) - 1.0f) : (1.0f - std::sqrt(2.0f - r2));

                                float di = (sx + 0.5f + tx) / _superSampleRate;
                                float dj = (sy + 0.5f + ty) / _superSampleRate;

                                glm::vec3 rayDir = lookDir;
                                rayDir += fovFactor * (2.0f * (j + dj) / height - 1.0f) * upDir;
                                rayDir += fovFactor * aspect * (2.0f * (i + di) / width - 1.0f) * rightDir;

                                Ray initialRay(camera.Eye, glm::normalize(rayDir));

                                subPixelSum += PathRayTrace(_intersector, initialRay, _maximumDepth, _enableShadow);
                            }

                            pixelSum += (subPixelSum / (float) samplesPerSubPixel);
                        }
                    }

                    glm::vec3 finalLinearColor = pixelSum / (float) (_superSampleRate * _superSampleRate);
                    _buffer.At(i, j)           = glm::pow(finalLinearColor, glm::vec3(1.0 / 2.2));
